...
# # Define the path for the persistent database and the embedding model
# CHROMA_PATH = "chroma_db"
# EMBEDDING_FUNCTION = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")

# def add_document_to_kb(doc_url: str):
#     """Downloads, processes, and adds a new document to the knowledge base."""
#     print(f"Adding document from {doc_url} to the knowledge base...")
...
#     # Load existing DB or create a new one, and add the new texts
...
#     print(f"Successfully added {len(chunks)} chunks from {doc_url}.")

# async def get_answers_from_kb(questions: List[str]) -> List[Dict]:
#     """Gets answers by searching the entire knowledge base."""
#     print("Setting up the RAG chain with local Ollama and ChromaDB...")
...
#     print("RAG chain is ready. Generating answers concurrently...")
#     tasks = [retrieval_chain.ainvoke({"input": q}) for q in questions]
#     results = await asyncio.gather(*tasks)
#     final_results = [
#         {"answer": res['answer'], "sources": res.get('context', [])}
#         for res in results
#     ]
...
async def get_answers_from_kb(questions: List[str]) -> List[Dict]:
    ...
    # This prompt instructs the LLM to return a JSON object.
    prompt = ChatPromptTemplate.from_template(
    ...
    async def get_structured_answer(question: str) -> Dict:
        ...
        try:
            ...
            if json_start_index != -1 and json_end_index != 0:
                ...
                # Ensure the 'answer' field is a string, not another object
